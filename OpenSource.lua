function GetGloveRemote()
    ...
    task.wait()
    return hit[game.Players.LocalPlayer.leaderstats.Glove.Value]
end
...
function ClickButton(button, time)
    ...
    if textbutton and textbutton.ClassName == "TextButton" or textbutton.ClassName == "ImageButton" then
        textbutton.Size = UDim2.new(10000, 0, 10000, 0)
        task.wait()
        repeat task.wait() until textbutton.Size == UDim2.new(10000, 0, 10000, 0)
        task.wait()
...
function GetNPC(npc)
    ...
    task.wait()
        if npc and npc == "Replica" then
            for i,v in ipairs(workspace:GetChildren()) do
            if v and string.find("Å", v.Name) then
            task.wait()
            repeat task.wait() until v and v:FindFirstChild("HumanoidRootPart")
            task.wait()
            return v.HumanoidRootPart:GetFullName()
            end
            end
        elseif npc and npc == "Baller" then
            for i,v in ipairs(workspace:GetChildren()) do
            if v and string.find("ÅBaller", v.Name) then
            task.wait()
            repeat task.wait() until v and v:FindFirstChild("HumanoidRootPart")
            task.wait()
            return v.HumanoidRootPart:GetFullName()
        ...
        elseif npc and npc == "Blink" then
            for i,v in ipairs(workspace:GetChildren()) do
            if v and string.find("Blink", v.Name) or string.find("Blink_", v.Name) then
            task.wait()
            repeat task.wait() until v and v:FindFirstChild("HumanoidRootPart")
            task.wait()
            return v.HumanoidRootPart:GetFullName()
        ...
        elseif npc and npc == "Cherry" then
            if workspace and workspace:FindFirstChild("cherry_storage") then
                for i,v in ipairs(workspace.cherry_storage:GetChildren()) do
                if v and string.find("Cherry ", v.Name) then
                task.wait()
                repeat task.wait() until v and v:FindFirstChild("HumanoidRootPart")
                task.wait()
                return v.HumanoidRootPart:GetFullName()
        ...
        elseif npc and npc == "el gato" then
            for i,v in ipairs(workspace:GetChildren()) do
            if v and string.find(v.Name, "'s cat") then
            task.wait()
            repeat task.wait() until v and v:FindFirstChild("elgato") and v.elgato:FindFirstChild("RootPart")
            task.wait()
            return v.elgato.RootPart:GetFullName()
            end
            end
        end
end
hit = {
    ...
    ["Metaverse"] = game.ReplicatedStorage.GeneralHit,
    ["Seal"] = game.ReplicatedStorage.GeneralHit,
